...
class FmsWalletTradeUsdtServices
{
    ...
    */
    public function getList($in_list,$start_timestamp,$nexturl='0'){
        if($nexturl != '0'){
            ...
            $url = 'https://api.trongrid.io/v1/accounts/'.$in_list['recharge_wallet_addr'].'/transactions/trc20?limit=50&only_to=true&min_timestamp='.$start_timestamp.'&contract_address=TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t';
        }
    ...
    public function handleWalletData($data,$in_list){
        if(isset($data['data'])){
            ...
            if($list){
                ...
                // $this->log('getpremiumwalletusdttrade','钱包地址：'.$in_list['recharge_wallet_addr'].'，成功hash值'.$success_hash_list_count.'条，失败hash值'.$error_hash_list_count.'条');
                // $this->log('getpremiumwalletusdttrade',json_encode($log_list,JSON_UNESCAPED_UNICODE));
