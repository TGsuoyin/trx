...
class PremiumWalletTradeUsdtServices
{
    ...
    public function handleWalletData($data,$in_list){
        if(isset($data['data'])){
            ...
            if($list){
                ...
                $transaction_hash_list = PremiumWalletTradeList::whereIn('tx_hash',$hash_list)->pluck('tx_hash')->toArray();
                ...
                // $this->log('getpremiumwalletusdttrade','钱包地址：'.$in_list['receive_wallet'].'，成功hash值'.$success_hash_list_count.'条，失败hash值'.$error_hash_list_count.'条');
    ...
    */
    public function AddWalletData($data,$time,$in_list){
        ...
        $txid_list['process_comments'] = '待开通';      //处理备注
        $txid_list['process_time'] = $time;        //处理时间
        
        PremiumWalletTradeList::insert($txid_list);       //添加收款钱包交易列表
